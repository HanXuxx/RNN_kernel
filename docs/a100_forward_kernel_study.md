...
A100 专用 forward-only 优化有效，已经快于 cuDNN forward；但还没有达到替代
`torch.nn.GRU` 训练的门槛，因为 backward 尚未实现。
...
fused backward step kernel、cooperative split backward step，以及多个负向实验已完成，见
...
4. backward 已完成第一轮有效优化：pointwise CUDA kernel、recurrent gate 聚合、
   ...
   当前 h256 训练主线已经推进到 split16 persistent-state gate-cache tiled，seq8000
   timed_steps=10 为 `137.471 ms/step`。下一步需要优化 forward gate-cache kernel、
   backward split16 gate-cache tiled kernel，以及 reserve-space 带来的显存/带宽成本。
   本轮已验证 gate-cache parallel-update、CTA8 forward、CTA6 forward、split32
   gate-cache tiled 和 grad-coeff-cache tiled 都没有超过当前主线，CTA4 shmem 结构仍是
   forward 主线。
