...
HIDDEN_SIZE = 256
_CUBIN_PACKAGE = "rnn_kernel.a100.prod.kernels"
...
def _require_cuda_python() -> None:
    if cuda is None:
        raise RuntimeError(
            "rnn_kernel.a100.prod requires cuda-python at runtime to load prebuilt cubin."
        )
...
def _load_prod_kernels(device_index: int) -> _ProdKernels:
    ...
    if not cubin_resource.is_file():
        raise RuntimeError(
            ...
            "`python scripts/build_a100_prod_cubin.py` in the build environment and include "
