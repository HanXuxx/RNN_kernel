...
extern "C" __global__
void a100_gru_h256_stacked_forward_naive_kernel(
    ...
    const float* __restrict__ bias_ih_l2,
    const float* __restrict__ bias_hh_l2,
    const float* __restrict__ weight_ih_l3,
    const float* __restrict__ weight_hh_l3,
    const float* __restrict__ bias_ih_l3,
    const float* __restrict__ bias_hh_l3,
    ...
    int num_layers)
{
    ...
    __shared__ float hidden_layers[MAX_LAYERS * HIDDEN_SIZE];
    __shared__ float layer_input[HIDDEN_SIZE];
    __shared__ float next_hidden[HIDDEN_SIZE];
    ...
    const float* bias_ih_layers[MAX_LAYERS] = {
        bias_ih_l0,
        bias_ih_l1,
        bias_ih_l2,
        bias_ih_l3,
    };
    const float* bias_hh_layers[MAX_LAYERS] = {
        bias_hh_l0,
        bias_hh_l1,
        bias_hh_l2,
        bias_hh_l3,
    ...
    for (int layer = 0; layer < num_layers; ++layer) {
        float* hidden = hidden_layers + layer * HIDDEN_SIZE;
        for (int hid = tid; hid < HIDDEN_SIZE; hid += blockDim.x) {
            hidden[hid] = h0[(layer * batch_size + batch_idx) * HIDDEN_SIZE + hid];
        }
    }
    __syncthreads();
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        for (int layer = 0; layer < num_layers; ++layer) {
            ...
            const float* bias_ih = bias_ih_layers[layer];
            const float* bias_hh = bias_hh_layers[layer];
            float* hidden = hidden_layers + layer * HIDDEN_SIZE;
            ...
            for (int hid = tid; hid < HIDDEN_SIZE; hid += blockDim.x) {
                ...
                if (layer == 0) {
                    for (int k = 0; k < layer_input_size; ++k) {
                        ...
                        i_r += x_value * weight_ih[hid * layer_input_size + k];
                        i_z += x_value * weight_ih[(HIDDEN_SIZE + hid) * layer_input_size + k];
                        i_n += x_value * weight_ih[(2 * HIDDEN_SIZE + hid) * layer_input_size + k];
                    }
                } else {
                    for (int k = 0; k < HIDDEN_SIZE; ++k) {
                        const float x_value = layer_input[k];
                        i_r += x_value * weight_ih[hid * HIDDEN_SIZE + k];
                        i_z += x_value * weight_ih[(HIDDEN_SIZE + hid) * HIDDEN_SIZE + k];
                        i_n += x_value * weight_ih[(2 * HIDDEN_SIZE + hid) * HIDDEN_SIZE + k];
                    }
                }

                float h_r = bias_hh[hid];
                float h_z = bias_hh[HIDDEN_SIZE + hid];
                float h_n_value = bias_hh[2 * HIDDEN_SIZE + hid];
                for (int k = 0; k < HIDDEN_SIZE; ++k) {
                    ...
                    h_n_value += hidden_value * weight_hh[(2 * HIDDEN_SIZE + hid) * HIDDEN_SIZE + k];
                }

                const float reset_gate = 1.0f / (1.0f + expf(-(i_r + h_r)));
                const float update_gate = 1.0f / (1.0f + expf(-(i_z + h_z)));
                const float new_gate = tanhf(i_n + reset_gate * h_n_value);
            ...
            for (int hid = tid; hid < HIDDEN_SIZE; hid += blockDim.x) {
                ...
                layer_input[hid] = hidden_next;
                if (step == seq_len - 1) {
                    h_n[(layer * batch_size + batch_idx) * HIDDEN_SIZE + hid] = hidden_next;
                }
...
extern "C" __global__
void a100_gru_forward_from_gates_cooperative_h256_htile4_compact_hoist_row4_no_cache_k1_kernel(
    const float* __restrict__ input_gates,
    const float* __restrict__ h0,
    const float* __restrict__ weight_hh,
    const float* __restrict__ bias_hh,
    ...
    int seq_len)
{
    ...
    constexpr int H_TILES = 4;
    constexpr int H_TILE = HIDDEN_SIZE / H_TILES;
    constexpr int CTAS_PER_BATCH = H_TILES;
    constexpr int GROUPS_PER_BLOCK = 16;
    ...
    const int global_cta = blockIdx.x;
    const int batch_idx = global_cta >> 2;
    const int h_tile_idx = global_cta & (CTAS_PER_BATCH - 1);
    const int tid = threadIdx.x;
    const int warp_lane = tid & (WARP_SIZE - 1);
    const int lane = tid & 15;
    const int group_idx = tid >> 4;
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        // K1 分支完全去掉跨 K CTA partial，但每个 CTA 负责完整 hidden dot。
#pragma unroll
        for (int pair_idx = 0; pair_idx < 2; ++pair_idx) {
            const int pair_base = pair_idx * 2 * GROUPS_PER_BLOCK;
            const int h0_local = group_idx + pair_base;
            ...
            const float* weight_r0 = weight_hh + hid0 * HIDDEN_SIZE;
            const float* weight_z0 = weight_hh + (HIDDEN_SIZE + hid0) * HIDDEN_SIZE;
            const float* weight_n0 = weight_hh + (2 * HIDDEN_SIZE + hid0) * HIDDEN_SIZE;
            const float* weight_r1 = weight_hh + hid1 * HIDDEN_SIZE;
            const float* weight_z1 = weight_hh + (HIDDEN_SIZE + hid1) * HIDDEN_SIZE;
            const float* weight_n1 = weight_hh + (2 * HIDDEN_SIZE + hid1) * HIDDEN_SIZE;
...
#pragma unroll
            for (int k_local = lane; k_local < HIDDEN_SIZE; k_local += 16) {
                const float hidden_value = hidden[k_local];
                acc_r0 += hidden_value * weight_r0[k_local];
                acc_z0 += hidden_value * weight_z0[k_local];
                acc_n0 += hidden_value * weight_n0[k_local];
                acc_r1 += hidden_value * weight_r1[k_local];
            ...
            if (lane == 0) {
                partial0_local[h0_local] = acc_r0;
                partial0_local[H_TILE + h0_local] = acc_z0;
                partial0_local[2 * H_TILE + h0_local] = acc_n0;
                partial0_local[h1_local] = acc_r1;
                partial0_local[H_TILE + h1_local] = acc_z1;
                partial0_local[2 * H_TILE + h1_local] = acc_n1;
            }
        }
        __syncthreads();
        grid.sync();
        ...
        for (int h_local = tid; h_local < H_TILE; h_local += blockDim.x) {
            const int hid = h_begin + h_local;
            const float acc_r = bias_hh[hid] + partial0_local[h_local];
            const float acc_z = bias_hh[HIDDEN_SIZE + hid] + partial0_local[H_TILE + h_local];
            const float acc_n = bias_hh[2 * HIDDEN_SIZE + hid] + partial0_local[2 * H_TILE + h_local];
...
extern "C" __global__
void a100_gru_forward_from_gates_cooperative_h256_htile8_compact_hoist_k1_no_cache_kernel(
    const float* __restrict__ input_gates,
    const float* __restrict__ h0,
    const float* __restrict__ weight_hh,
    const float* __restrict__ bias_hh,
    ...
    int seq_len)
{
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        // htile8/K1 用更多 CTA 覆盖 batch=16，减少 A100 SM 空置。
...
extern "C" __global__
void a100_gru_forward_from_gates_cooperative_h256_htile4_compact_hoist_row4_k1_shmem_gate_cache_kernel(
    const float* __restrict__ input_gates,
    const float* __restrict__ h0,
    const float* __restrict__ weight_hh,
    const float* __restrict__ bias_hh,
    ...
    int seq_len)
{
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        // K1 gate-cache 分支保留 backward 所需 cache，但去掉跨 K CTA partial。
#pragma unroll
        for (int pair_idx = 0; pair_idx < 2; ++pair_idx) {
            const int pair_base = pair_idx * 2 * GROUPS_PER_BLOCK;
            const int h0_local = group_idx + pair_base;
...
extern "C" __global__
void a100_gru_forward_from_gates_cooperative_h256_htile8_compact_hoist_k1_shmem_gate_cache_kernel(
    const float* __restrict__ input_gates,
    const float* __restrict__ h0,
    const float* __restrict__ weight_hh,
    const float* __restrict__ bias_hh,
    ...
    int seq_len)
{
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        // htile8/K1 gate-cache 分支保留 backward cache，同时增加 batch=16 resident CTA。
        const int h0_local = group_idx;
        const int h1_local = h0_local + GROUPS_PER_BLOCK;
        const int hid0 = h_begin + h0_local;
        const int hid1 = h_begin + h1_local;
...
extern "C" __global__
void a100_gru_forward_from_gates_cooperative_h256_htile4_compact_hoist_row4_no_cache_k2_kernel(
    const float* __restrict__ input_gates,
    const float* __restrict__ h0,
    const float* __restrict__ weight_hh,
    const float* __restrict__ bias_hh,
    ...
    int seq_len)
{
    ...
    const int batch_idx = global_cta >> 3;
    const int cta_idx = global_cta & (CTAS_PER_BATCH - 1);
    const int h_tile_idx = cta_idx >> 1;
    const int k_cta_idx = cta_idx & (K_CTAS - 1);
    const int tid = threadIdx.x;
    const int warp_lane = tid & (WARP_SIZE - 1);
    const int lane = tid & 15;
    ...
    const int h_begin = h_tile_idx * H_TILE;
    const int k_begin = k_cta_idx * K_TILE;
    const int partial_base_idx = batch_idx * COMPACT_PARTIALS_PER_BATCH + h_tile_idx;
    float* partial_out_base = partial0_local;
    if (k_cta_idx != 0) {
        partial_out_base = partial_gates + partial_base_idx * PARTIAL_STRIDE;
    }
    const float* partial1_base = partial_gates + partial_base_idx * PARTIAL_STRIDE;
    float* hidden = hidden_state + batch_idx * HIDDEN_SIZE;
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        // K2 分支减少每步参与 grid.sync 的 CTA 数，但每个 CTA 承担更长 K 维 dot。
#pragma unroll
        for (int pair_idx = 0; pair_idx < 2; ++pair_idx) {
            const int pair_base = pair_idx * 2 * GROUPS_PER_BLOCK;
            const int h0_local = group_idx + pair_base;
...
extern "C" __global__
void a100_gru_forward_from_gates_cooperative_h256_htile4_compact_hoist_row4_no_cache_ldg_kernel(
    const float* __restrict__ input_gates,
    const float* __restrict__ h0,
    const float* __restrict__ weight_hh,
    const float* __restrict__ bias_hh,
    ...
    int seq_len)
{
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        // no-cache 推理路径只对真正只读的 input/weight/bias 使用只读加载。
#pragma unroll
        for (int pair_idx = 0; pair_idx < 2; ++pair_idx) {
            const int pair_base = pair_idx * 2 * GROUPS_PER_BLOCK;
            const int h0_local = group_idx + pair_base;
...
extern "C" __global__
void a100_gru_h256_stacked_row4_forward_kernel(
    const float* __restrict__ x,
    const float* __restrict__ h0,
    const float* __restrict__ weight_ih_l0,
    const float* __restrict__ weight_hh_l0,
    ...
    float* __restrict__ partial_gates,
    float* __restrict__ hidden_state,
    float* __restrict__ layer_input_state,
    float* __restrict__ output,
    float* __restrict__ h_n,
    int batch_size,
    int seq_len,
    ...
    int num_layers)
{
    ...
    constexpr int CTAS_PER_BATCH = H_TILES * K_CTAS;
    constexpr int COMPACT_PARTIALS_PER_BATCH = H_TILES * (K_CTAS - 1);
    constexpr int PARTIAL_STRIDE = 4 * H_TILE;
    constexpr int GROUPS_PER_BLOCK = 16;
    constexpr int MAX_LAYERS = 4;
    ...
    for (int layer = 0; layer < num_layers; ++layer) {
        float* hidden = hidden_state
            + (batch_idx * num_layers + layer) * HIDDEN_SIZE;
        if (k_cta_idx == 0) {
            for (int h_local = tid; h_local < H_TILE; h_local += blockDim.x) {
                const int hid = h_begin + h_local;
                hidden[hid] = h0[(layer * batch_size + batch_idx) * HIDDEN_SIZE + hid];
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        for (int layer = 0; layer < num_layers; ++layer) {
            ...
            const float* layer_input = (layer == 0)
                ? nullptr
                : hidden_state + (batch_idx * num_layers + layer - 1) * HIDDEN_SIZE;
            float* output_step = output + (batch_idx * seq_len + step) * HIDDEN_SIZE;

            // layer>0 时把 input projection 和 recurrent projection 合并到同一次 K 分片 reduce。
#pragma unroll
            for (int pair_idx = 0; pair_idx < 2; ++pair_idx) {
                const int pair_base = pair_idx * 2 * GROUPS_PER_BLOCK;
                const int h0_local = group_idx + pair_base;
                ...
                const float* weight_hh_r0 = weight_hh + hid0 * HIDDEN_SIZE + k_begin;
                const float* weight_hh_z0 = weight_hh + (HIDDEN_SIZE + hid0) * HIDDEN_SIZE + k_begin;
                const float* weight_hh_n0 = weight_hh + (2 * HIDDEN_SIZE + hid0) * HIDDEN_SIZE + k_begin;
                const float* weight_hh_r1 = weight_hh + hid1 * HIDDEN_SIZE + k_begin;
                const float* weight_hh_z1 = weight_hh + (HIDDEN_SIZE + hid1) * HIDDEN_SIZE + k_begin;
                const float* weight_hh_n1 = weight_hh + (2 * HIDDEN_SIZE + hid1) * HIDDEN_SIZE + k_begin;

                const float* weight_ih_r0 = weight_ih + hid0 * HIDDEN_SIZE + k_begin;
                const float* weight_ih_z0 = weight_ih + (HIDDEN_SIZE + hid0) * HIDDEN_SIZE + k_begin;
                const float* weight_ih_n0 = weight_ih + (2 * HIDDEN_SIZE + hid0) * HIDDEN_SIZE + k_begin;
                const float* weight_ih_r1 = weight_ih + hid1 * HIDDEN_SIZE + k_begin;
                const float* weight_ih_z1 = weight_ih + (HIDDEN_SIZE + hid1) * HIDDEN_SIZE + k_begin;
                const float* weight_ih_n1 = weight_ih + (2 * HIDDEN_SIZE + hid1) * HIDDEN_SIZE + k_begin;

                float acc_r0 = 0.0f;
                float acc_z0 = 0.0f;
                float acc_n0 = 0.0f;
                float acc_input_n0 = 0.0f;
                float acc_r1 = 0.0f;
                float acc_z1 = 0.0f;
                float acc_n1 = 0.0f;
                float acc_input_n1 = 0.0f;

#pragma unroll
                for (int k_local = lane; k_local < K_TILE; k_local += 16) {
                    const int k = k_begin + k_local;
                    const float hidden_value = hidden[k];
                    acc_r0 += hidden_value * weight_hh_r0[k_local];
                    acc_z0 += hidden_value * weight_hh_z0[k_local];
                    acc_n0 += hidden_value * weight_hh_n0[k_local];
                    acc_r1 += hidden_value * weight_hh_r1[k_local];
                    acc_z1 += hidden_value * weight_hh_z1[k_local];
                    acc_n1 += hidden_value * weight_hh_n1[k_local];

                    if (layer != 0) {
                        const float input_value = layer_input[k];
                        acc_r0 += input_value * weight_ih_r0[k_local];
                        acc_z0 += input_value * weight_ih_z0[k_local];
                        acc_input_n0 += input_value * weight_ih_n0[k_local];
                        acc_r1 += input_value * weight_ih_r1[k_local];
                        acc_z1 += input_value * weight_ih_z1[k_local];
                        acc_input_n1 += input_value * weight_ih_n1[k_local];
                    }
                }
                ...
                acc_z0 = half_warp_reduce_sum(acc_z0, group_mask);
                acc_n0 = half_warp_reduce_sum(acc_n0, group_mask);
                acc_input_n0 = half_warp_reduce_sum(acc_input_n0, group_mask);
                acc_r1 = half_warp_reduce_sum(acc_r1, group_mask);
                acc_z1 = half_warp_reduce_sum(acc_z1, group_mask);
                acc_n1 = half_warp_reduce_sum(acc_n1, group_mask);
                acc_input_n1 = half_warp_reduce_sum(acc_input_n1, group_mask);

                if (lane == 0) {
                    partial_out_base[h0_local] = acc_r0;
                    partial_out_base[H_TILE + h0_local] = acc_z0;
                    partial_out_base[2 * H_TILE + h0_local] = acc_n0;
                    partial_out_base[3 * H_TILE + h0_local] = acc_input_n0;
                    partial_out_base[h1_local] = acc_r1;
                    partial_out_base[H_TILE + h1_local] = acc_z1;
                    partial_out_base[2 * H_TILE + h1_local] = acc_n1;
                    partial_out_base[3 * H_TILE + h1_local] = acc_input_n1;
                }
            }
            grid.sync();
            ...
            if (k_cta_idx == 0) {
                for (int h_local = tid; h_local < H_TILE; h_local += blockDim.x) {
                    const int hid = h_begin + h_local;
                    float gate_r = bias_hh[hid]
                        + bias_ih[hid]
                        + partial0_local[h_local]
                        + partial1_base[h_local]
                        + partial2_base[h_local]
                        + partial3_base[h_local];
                    float gate_z = bias_hh[HIDDEN_SIZE + hid]
                        + bias_ih[HIDDEN_SIZE + hid]
                        + partial0_local[H_TILE + h_local]
                        + partial1_base[H_TILE + h_local]
                        + partial2_base[H_TILE + h_local]
                        + partial3_base[H_TILE + h_local];
                    float gate_n_input = bias_ih[2 * HIDDEN_SIZE + hid];
                    float gate_n_recurrent = bias_hh[2 * HIDDEN_SIZE + hid]
                        + partial0_local[2 * H_TILE + h_local]
                        + partial1_base[2 * H_TILE + h_local]
                        + partial2_base[2 * H_TILE + h_local]
                        + partial3_base[2 * H_TILE + h_local];
                    ...
                    if (layer == 0) {
                        for (int k = 0; k < input_size; ++k) {
                            const float input_value = x_step[k];
                            gate_r += input_value * weight_ih[hid * input_size + k];
                            gate_z += input_value * weight_ih[(HIDDEN_SIZE + hid) * input_size + k];
                            gate_n_input += input_value * weight_ih[(2 * HIDDEN_SIZE + hid) * input_size + k];
                        }
                    } else {
                        gate_n_input += partial0_local[3 * H_TILE + h_local]
                            + partial1_base[3 * H_TILE + h_local]
                            + partial2_base[3 * H_TILE + h_local]
                            + partial3_base[3 * H_TILE + h_local];
                    }

                    const float reset_gate = 1.0f / (1.0f + expf(-gate_r));
                    const float update_gate = 1.0f / (1.0f + expf(-gate_z));
                    const float new_gate = tanhf(gate_n_input + reset_gate * gate_n_recurrent);
                    const float hidden_prev = hidden[hid];
                    const float hidden_next = new_gate + update_gate * (hidden_prev - new_gate);
                    hidden[hid] = hidden_next;
                    if (step == seq_len - 1) {
...
void a100_gru_h256_stacked_row4_train_forward_kernel(
    const float* __restrict__ x,
    const float* __restrict__ h0,
    const float* __restrict__ weight_ih_l0,
    const float* __restrict__ weight_hh_l0,
    ...
    float* __restrict__ gate_cache_all,
    int batch_size,
    int seq_len,
    int input_size,
    int num_layers)
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        for (int layer = 0; layer < num_layers; ++layer) {
            ...
            const float* layer_input = (layer == 0)
                ...
                : hidden_state + (batch_idx * num_layers + layer - 1) * HIDDEN_SIZE;
            float* output_step = output + (batch_idx * seq_len + step) * HIDDEN_SIZE;
            float* layer_output_step = all_outputs
                + ((layer * batch_size + batch_idx) * seq_len + step) * HIDDEN_SIZE;
            float* cache_step = gate_cache_all
                + ((layer * batch_size + batch_idx) * seq_len + step) * CACHE_SIZE;

            // 训练版保留每层 output 和 gate cache，便于后续 fused backward 复用。
#pragma unroll
            for (int pair_idx = 0; pair_idx < 2; ++pair_idx) {
                const int pair_base = pair_idx * 2 * GROUPS_PER_BLOCK;
                const int h0_local = group_idx + pair_base;
...
#pragma unroll
            ...
            if (k_cta_idx == 0) {
                for (int h_local = tid; h_local < H_TILE; h_local += blockDim.x) {
                    ...
                    const float hidden_next = new_gate + update_gate * (hidden_prev - new_gate);
                    hidden[hid] = hidden_next;
                    layer_output_step[hid] = hidden_next;
                    cache_step[hid] = reset_gate;
                    cache_step[HIDDEN_SIZE + hid] = update_gate;
                    cache_step[2 * HIDDEN_SIZE + hid] = new_gate;
                    cache_step[3 * HIDDEN_SIZE + hid] = gate_n_recurrent;
                    if (step == seq_len - 1) {
                        h_n[(layer * batch_size + batch_idx) * HIDDEN_SIZE + hid] = hidden_next;
                    }
                    if (layer == num_layers - 1) {
...
extern "C" __global__ __launch_bounds__(256, 3)
void a100_gru_h256_stacked_row4_k1_train_forward_kernel(
    const float* __restrict__ x,
    const float* __restrict__ h0,
    const float* __restrict__ weight_ih_l0,
    const float* __restrict__ weight_hh_l0,
    ...
    int num_layers)
{
    ...
    for (int step = 0; step < seq_len; ++step) {
        ...
        for (int layer = 0; layer < num_layers; ++layer) {
            ...
            // K1 stacked forward 去掉跨 K CTA partial，减少每层一次 grid-wide reduce 同步。
#pragma unroll
            for (int pair_idx = 0; pair_idx < 2; ++pair_idx) {
                const int pair_base = pair_idx * 2 * GROUPS_PER_BLOCK;
                const int h0_local = group_idx + pair_base;
                ...
                const float* weight_hh_r0 = weight_hh + hid0 * HIDDEN_SIZE;
                const float* weight_hh_z0 = weight_hh + (HIDDEN_SIZE + hid0) * HIDDEN_SIZE;
                const float* weight_hh_n0 = weight_hh + (2 * HIDDEN_SIZE + hid0) * HIDDEN_SIZE;
                const float* weight_hh_r1 = weight_hh + hid1 * HIDDEN_SIZE;
                const float* weight_hh_z1 = weight_hh + (HIDDEN_SIZE + hid1) * HIDDEN_SIZE;
                const float* weight_hh_n1 = weight_hh + (2 * HIDDEN_SIZE + hid1) * HIDDEN_SIZE;

                const float* weight_ih_r0 = weight_ih + hid0 * HIDDEN_SIZE;
                const float* weight_ih_z0 = weight_ih + (HIDDEN_SIZE + hid0) * HIDDEN_SIZE;
                const float* weight_ih_n0 = weight_ih + (2 * HIDDEN_SIZE + hid0) * HIDDEN_SIZE;
                const float* weight_ih_r1 = weight_ih + hid1 * HIDDEN_SIZE;
                const float* weight_ih_z1 = weight_ih + (HIDDEN_SIZE + hid1) * HIDDEN_SIZE;
                const float* weight_ih_n1 = weight_ih + (2 * HIDDEN_SIZE + hid1) * HIDDEN_SIZE;
...
#pragma unroll
                for (int k = lane; k < HIDDEN_SIZE; k += 16) {
                    const float hidden_value = hidden[k];
                    acc_r0 += hidden_value * weight_hh_r0[k];
                    acc_z0 += hidden_value * weight_hh_z0[k];
                    acc_n0 += hidden_value * weight_hh_n0[k];
                    acc_r1 += hidden_value * weight_hh_r1[k];
                    acc_z1 += hidden_value * weight_hh_z1[k];
                    acc_n1 += hidden_value * weight_hh_n1[k];

                    if (layer != 0) {
                        const float input_value = layer_input[k];
                        acc_r0 += input_value * weight_ih_r0[k];
                        acc_z0 += input_value * weight_ih_z0[k];
                        acc_input_n0 += input_value * weight_ih_n0[k];
                        acc_r1 += input_value * weight_ih_r1[k];
                        acc_z1 += input_value * weight_ih_z1[k];
                        acc_input_n1 += input_value * weight_ih_n1[k];
                    }
                }
                ...
                if (lane == 0) {
                    ...
                    partial0_local[H_TILE + h0_local] = acc_z0;
                    partial0_local[2 * H_TILE + h0_local] = acc_n0;
                    partial0_local[3 * H_TILE + h0_local] = acc_input_n0;
                    partial0_local[h1_local] = acc_r1;
                    partial0_local[H_TILE + h1_local] = acc_z1;
                    partial0_local[2 * H_TILE + h1_local] = acc_n1;
                    partial0_local[3 * H_TILE + h1_local] = acc_input_n1;
                }
            }
            __syncthreads();
            ...
            for (int h_local = tid; h_local < H_TILE; h_local += blockDim.x) {
                const int hid = h_begin + h_local;
                float gate_r = bias_hh[hid] + bias_ih[hid] + partial0_local[h_local];
                float gate_z = bias_hh[HIDDEN_SIZE + hid]
                    + bias_ih[HIDDEN_SIZE + hid]
                    + partial0_local[H_TILE + h_local];
                float gate_n_input = bias_ih[2 * HIDDEN_SIZE + hid];
            ...
            if (layer != num_layers - 1 || num_layers == 1) {
                grid.sync();
            }
        }
    }
...
void a100_gru_h256_stacked_backward_naive_kernel(
    ...
    int num_layers)
{
    ...
    if (batch_idx >= batch_size || hid >= HIDDEN_SIZE || num_layers < 1 || num_layers > MAX_LAYERS) {
        return;
    }
    ...
    for (int layer = num_layers - 1; layer >= 0; --layer) {
        const float* weight_ih = weight_ih_layers[layer];
        const float* weight_hh = weight_hh_layers[layer];
        float grad_hidden = grad_h_n[(layer * batch_size + batch_idx) * HIDDEN_SIZE + hid];

        for (int step = seq_len - 1; step >= 0; --step) {
            const int output_offset = ((layer * batch_size + batch_idx) * seq_len + step) * HIDDEN_SIZE;
            const int gate_offset = ((layer * batch_size + batch_idx) * seq_len + step) * GATE_SIZE;
            const int cache_offset = ((layer * batch_size + batch_idx) * seq_len + step) * CACHE_SIZE;
            const int batch_step_hidden_offset = (batch_idx * seq_len + step) * HIDDEN_SIZE;
            const int batch_step_input_offset = (batch_idx * seq_len + step) * input_size;
            ...
            const float grad_hidden_next = grad_hidden + direct_grad;

            const float hidden_next = all_outputs[output_offset + hid];
            const float hidden_prev = (step == 0)
                ? h0[(layer * batch_size + batch_idx) * HIDDEN_SIZE + hid]
            ...
            const float grad_hidden_prev_direct = grad_hidden_next * update_gate;
            const float grad_new_pre = grad_new * (1.0f - new_gate * new_gate);
            const float grad_input_n = grad_new_pre;
            ...
            grad_input_gates_all[gate_offset + 2 * HIDDEN_SIZE + hid] = grad_input_n;
            ...
            float recurrent_grad = 0.0f;
            float input_grad = 0.0f;
#pragma unroll
            for (int gate = 0; gate < 3; ++gate) {
                const float* grad_hidden_gate_base = grad_hidden_gates_all
                    + gate_offset + gate * HIDDEN_SIZE;
                const float* grad_input_gate_base = grad_input_gates_all
                    + gate_offset + gate * HIDDEN_SIZE;
                const float* weight_hh_gate = weight_hh + gate * HIDDEN_SIZE * HIDDEN_SIZE;
                const float* weight_ih_gate = weight_ih
                    + gate * HIDDEN_SIZE * (layer == 0 ? input_size : HIDDEN_SIZE);

                for (int row = 0; row < HIDDEN_SIZE; ++row) {
                    recurrent_grad += grad_hidden_gate_base[row]
                        * weight_hh_gate[row * HIDDEN_SIZE + hid];
                    if (layer != 0) {
                        input_grad += grad_input_gate_base[row]
                            * weight_ih_gate[row * HIDDEN_SIZE + hid];
                    } else if (hid < input_size) {
                        input_grad += grad_input_gate_base[row]
                            * weight_ih_gate[row * input_size + hid];
                    }
                }
            }

            grad_hidden = grad_hidden_prev_direct + recurrent_grad;
            if (layer == 0) {
                if (hid < input_size) {
                ...
                grad_layer_outputs[
                    (((layer - 1) * batch_size + batch_idx) * seq_len + step) * HIDDEN_SIZE + hid
                ] = input_grad;
            }
            __syncthreads();
        }

        grad_h0[(layer * batch_size + batch_idx) * HIDDEN_SIZE + hid] = grad_hidden;
        __syncthreads();
    }
}

extern "C" __global__ __launch_bounds__(256, 2)
void a100_gru_h256_stacked_backward_split4_kernel(
    const float* __restrict__ grad_output_top,
    const float* __restrict__ grad_h_n,
    const float* __restrict__ x,
    const float* __restrict__ h0,
    ...
    int num_layers)
{
    ...
    constexpr int H_TILE = HIDDEN_SIZE / SPLITS;
    constexpr int GROUP_SIZE = 4;

    cg::grid_group grid = cg::this_grid();

    const int global_cta = blockIdx.x;
    const int batch_idx = global_cta / SPLITS;
    const int split_idx = global_cta - batch_idx * SPLITS;
    const int tid = threadIdx.x;
    const int group_idx = tid / GROUP_SIZE;
    const int group_lane = tid & (GROUP_SIZE - 1);
    const int hid = split_idx * H_TILE + group_idx;
    ...
    for (int layer = num_layers - 1; layer >= 0; --layer) {
        ...
        if (group_lane == 0) {
            grad_hidden = grad_h_n[(layer * batch_size + batch_idx) * HIDDEN_SIZE + hid];
        }
        ...
        for (int step = seq_len - 1; step >= 0; --step) {
            ...
            if (group_lane == 0) {
                ...
                grad_hidden_gates_all[gate_offset + HIDDEN_SIZE + hid] = grad_input_z;
                grad_hidden_gates_all[gate_offset + 2 * HIDDEN_SIZE + hid] = grad_hidden_n;
                grad_hidden = grad_hidden_next * update_gate;
            }
            grid.sync();
...
#pragma unroll
            for (int gate = 0; gate < 3; ++gate) {
                ...
                for (int row = group_lane; row < HIDDEN_SIZE; row += GROUP_SIZE) {
                    recurrent_grad += grad_hidden_gate_base[row]
                        * weight_hh_gate[row * HIDDEN_SIZE + hid];
                    if (layer != 0) {
                        input_grad += grad_input_gate_base[row]
            ...
            recurrent_grad += __shfl_down_sync(0xffffffffu, recurrent_grad, 2);
            input_grad += __shfl_down_sync(0xffffffffu, input_grad, 2);
            recurrent_grad += __shfl_down_sync(0xffffffffu, recurrent_grad, 1);
            input_grad += __shfl_down_sync(0xffffffffu, input_grad, 1);

            if (group_lane == 0) {
                grad_hidden += recurrent_grad;
                if (layer == 0) {
                    if (hid < input_size) {
                        grad_x[batch_step_input_offset + hid] = input_grad;
                    }
...
extern "C" __global__ __launch_bounds__(256, 2)
void a100_gru_h256_stacked_backward_split4_group8_kernel(
    const float* __restrict__ grad_output_top,
    const float* __restrict__ grad_h_n,
    const float* __restrict__ x,
    const float* __restrict__ h0,
    ...
    int num_layers)
{
    ...
    constexpr int GROUPS_PER_BLOCK = 256 / GROUP_SIZE;
    ...
    const int group_idx = tid / GROUP_SIZE;
    const int group_lane = tid & (GROUP_SIZE - 1);
    const int h_begin = split_idx * H_TILE;
    const int hid0 = h_begin + group_idx;
    const int hid1 = h_begin + group_idx + GROUPS_PER_BLOCK;
    ...
    for (int layer = num_layers - 1; layer >= 0; --layer) {
        const float* weight_ih = weight_ih_layers[layer];
        const float* weight_hh = weight_hh_layers[layer];
        float grad_hidden0 = 0.0f;
        float grad_hidden1 = 0.0f;
        if (group_lane == 0) {
            const int h0_base = (layer * batch_size + batch_idx) * HIDDEN_SIZE;
            grad_hidden0 = grad_h_n[h0_base + hid0];
            grad_hidden1 = grad_h_n[h0_base + hid1];
        }
        ...
        for (int step = seq_len - 1; step >= 0; --step) {
            ...
            if (group_lane == 0) {
                const int h0_base = (layer * batch_size + batch_idx) * HIDDEN_SIZE;
                const float direct_grad0 = (layer == num_layers - 1)
                    ? grad_output_top[batch_step_hidden_offset + hid0]
                    : grad_layer_outputs[output_offset + hid0];
                const float direct_grad1 = (layer == num_layers - 1)
                    ? grad_output_top[batch_step_hidden_offset + hid1]
                    : grad_layer_outputs[output_offset + hid1];
                const float grad_hidden_next0 = grad_hidden0 + direct_grad0;
                const float grad_hidden_next1 = grad_hidden1 + direct_grad1;

                const float hidden_prev0 = (step == 0)
                    ? h0[h0_base + hid0]
                    : all_outputs[output_offset - HIDDEN_SIZE + hid0];
                const float hidden_prev1 = (step == 0)
                    ? h0[h0_base + hid1]
                    : all_outputs[output_offset - HIDDEN_SIZE + hid1];
                const float reset_gate0 = gate_cache_all[cache_offset + hid0];
                const float reset_gate1 = gate_cache_all[cache_offset + hid1];
                const float update_gate0 = gate_cache_all[cache_offset + HIDDEN_SIZE + hid0];
                const float update_gate1 = gate_cache_all[cache_offset + HIDDEN_SIZE + hid1];
                const float new_gate0 = gate_cache_all[cache_offset + 2 * HIDDEN_SIZE + hid0];
                const float new_gate1 = gate_cache_all[cache_offset + 2 * HIDDEN_SIZE + hid1];
                const float hidden_gate_n0 = gate_cache_all[cache_offset + 3 * HIDDEN_SIZE + hid0];
                const float hidden_gate_n1 = gate_cache_all[cache_offset + 3 * HIDDEN_SIZE + hid1];

                const float grad_new0 = grad_hidden_next0 * (1.0f - update_gate0);
                const float grad_new1 = grad_hidden_next1 * (1.0f - update_gate1);
                const float grad_update0 = grad_hidden_next0 * (hidden_prev0 - new_gate0);
                const float grad_update1 = grad_hidden_next1 * (hidden_prev1 - new_gate1);
                const float grad_new_pre0 = grad_new0 * (1.0f - new_gate0 * new_gate0);
                const float grad_new_pre1 = grad_new1 * (1.0f - new_gate1 * new_gate1);
                const float grad_hidden_n0 = grad_new_pre0 * reset_gate0;
                const float grad_hidden_n1 = grad_new_pre1 * reset_gate1;
                const float grad_reset0 = grad_new_pre0 * hidden_gate_n0;
                const float grad_reset1 = grad_new_pre1 * hidden_gate_n1;
                const float grad_input_r0 = grad_reset0 * reset_gate0 * (1.0f - reset_gate0);
                const float grad_input_r1 = grad_reset1 * reset_gate1 * (1.0f - reset_gate1);
                const float grad_input_z0 = grad_update0 * update_gate0 * (1.0f - update_gate0);
                const float grad_input_z1 = grad_update1 * update_gate1 * (1.0f - update_gate1);

                grad_input_gates_all[gate_offset + hid0] = grad_input_r0;
                grad_input_gates_all[gate_offset + hid1] = grad_input_r1;
                grad_input_gates_all[gate_offset + HIDDEN_SIZE + hid0] = grad_input_z0;
                grad_input_gates_all[gate_offset + HIDDEN_SIZE + hid1] = grad_input_z1;
                grad_input_gates_all[gate_offset + 2 * HIDDEN_SIZE + hid0] = grad_new_pre0;
                grad_input_gates_all[gate_offset + 2 * HIDDEN_SIZE + hid1] = grad_new_pre1;
                grad_hidden_gates_all[gate_offset + hid0] = grad_input_r0;
                grad_hidden_gates_all[gate_offset + hid1] = grad_input_r1;
                grad_hidden_gates_all[gate_offset + HIDDEN_SIZE + hid0] = grad_input_z0;
                grad_hidden_gates_all[gate_offset + HIDDEN_SIZE + hid1] = grad_input_z1;
                grad_hidden_gates_all[gate_offset + 2 * HIDDEN_SIZE + hid0] = grad_hidden_n0;
                grad_hidden_gates_all[gate_offset + 2 * HIDDEN_SIZE + hid1] = grad_hidden_n1;
                grad_hidden0 = grad_hidden_next0 * update_gate0;
                grad_hidden1 = grad_hidden_next1 * update_gate1;
            }
            grid.sync();

            float recurrent_grad0 = 0.0f;
            float recurrent_grad1 = 0.0f;
            float input_grad0 = 0.0f;
            float input_grad1 = 0.0f;
#pragma unroll
            for (int gate = 0; gate < 3; ++gate) {
                const float* grad_hidden_gate_base = grad_hidden_gates_all
                    + gate_offset + gate * HIDDEN_SIZE;
                ...
                for (int row = group_lane; row < HIDDEN_SIZE; row += GROUP_SIZE) {
                    const float grad_hidden_gate = grad_hidden_gate_base[row];
                    const float grad_input_gate = grad_input_gate_base[row];
                    recurrent_grad0 += grad_hidden_gate * weight_hh_gate[row * HIDDEN_SIZE + hid0];
                    recurrent_grad1 += grad_hidden_gate * weight_hh_gate[row * HIDDEN_SIZE + hid1];
                    if (layer != 0) {
                        input_grad0 += grad_input_gate * weight_ih_gate[row * HIDDEN_SIZE + hid0];
                        input_grad1 += grad_input_gate * weight_ih_gate[row * HIDDEN_SIZE + hid1];
                    } else {
                        if (hid0 < input_size) {
                            input_grad0 += grad_input_gate * weight_ih_gate[row * input_size + hid0];
                        }
                        if (hid1 < input_size) {
                            input_grad1 += grad_input_gate * weight_ih_gate[row * input_size + hid1];
                        }
                    }
                }
            }

            recurrent_grad0 += __shfl_down_sync(0xffffffffu, recurrent_grad0, 4);
            recurrent_grad1 += __shfl_down_sync(0xffffffffu, recurrent_grad1, 4);
            input_grad0 += __shfl_down_sync(0xffffffffu, input_grad0, 4);
            input_grad1 += __shfl_down_sync(0xffffffffu, input_grad1, 4);
            recurrent_grad0 += __shfl_down_sync(0xffffffffu, recurrent_grad0, 2);
            recurrent_grad1 += __shfl_down_sync(0xffffffffu, recurrent_grad1, 2);
            input_grad0 += __shfl_down_sync(0xffffffffu, input_grad0, 2);
            input_grad1 += __shfl_down_sync(0xffffffffu, input_grad1, 2);
            recurrent_grad0 += __shfl_down_sync(0xffffffffu, recurrent_grad0, 1);
            recurrent_grad1 += __shfl_down_sync(0xffffffffu, recurrent_grad1, 1);
            input_grad0 += __shfl_down_sync(0xffffffffu, input_grad0, 1);
            input_grad1 += __shfl_down_sync(0xffffffffu, input_grad1, 1);

            if (group_lane == 0) {
                grad_hidden0 += recurrent_grad0;
                grad_hidden1 += recurrent_grad1;
                if (layer == 0) {
                    if (hid0 < input_size) {
                        grad_x[batch_step_input_offset + hid0] = input_grad0;
                    }
                    if (hid1 < input_size) {
                        grad_x[batch_step_input_offset + hid1] = input_grad1;
                    }
                } else {
                    const int prev_output_offset =
                        (((layer - 1) * batch_size + batch_idx) * seq_len + step) * HIDDEN_SIZE;
                    grad_layer_outputs[prev_output_offset + hid0] = input_grad0;
                    grad_layer_outputs[prev_output_offset + hid1] = input_grad1;
                }
            }
        }
        ...
        if (group_lane == 0) {
            const int h0_base = (layer * batch_size + batch_idx) * HIDDEN_SIZE;
            grad_h0[h0_base + hid0] = grad_hidden0;
            grad_h0[h0_base + hid1] = grad_hidden1;
        }
        grid.sync();
    }
}
    ...
    int num_layers)
{
    ...
    for (int layer = num_layers - 1; layer >= 0; --layer) {
        ...
        // 每个 split 缓存 128x256 的 recurrent weight tile，沿时间步复用。
#pragma unroll 8
...
extern "C" __global__ __launch_bounds__(256, 2)
void a100_gru_h256_stacked_backward_split8_kernel(
    const float* __restrict__ grad_output_top,
    const float* __restrict__ grad_h_n,
    const float* __restrict__ x,
    const float* __restrict__ h0,
...
#pragma unroll
            ...
            recurrent_grad += __shfl_down_sync(0xffffffffu, recurrent_grad, 4);
            input_grad += __shfl_down_sync(0xffffffffu, input_grad, 4);
            recurrent_grad += __shfl_down_sync(0xffffffffu, recurrent_grad, 2);
            input_grad += __shfl_down_sync(0xffffffffu, input_grad, 2);
            recurrent_grad += __shfl_down_sync(0xffffffffu, recurrent_grad, 1);
            input_grad += __shfl_down_sync(0xffffffffu, input_grad, 1);
...
extern "C" __global__ __launch_bounds__(256, 2)
void a100_gru_h256_stacked_backward_split4_shmem_kernel(
    const float* __restrict__ grad_output_top,
    const float* __restrict__ grad_h_n,
    const float* __restrict__ x,
    const float* __restrict__ h0,
    ...
    int num_layers)
{
    ...
    __shared__ float sh_grad_input_gates[3 * H_TILE];
    __shared__ float sh_grad_hidden_gates[3 * H_TILE];
    cg::grid_group grid = cg::this_grid();
    ...
    for (int layer = num_layers - 1; layer >= 0; --layer) {
        ...
        for (int step = seq_len - 1; step >= 0; --step) {
            ...
            if (group_lane == 0) {
                ...
                const int h_local = group_idx;
                sh_grad_input_gates[h_local] = grad_input_r;
                sh_grad_input_gates[H_TILE + h_local] = grad_input_z;
                sh_grad_input_gates[2 * H_TILE + h_local] = grad_input_n;
                sh_grad_hidden_gates[h_local] = grad_input_r;
                sh_grad_hidden_gates[H_TILE + h_local] = grad_input_z;
                sh_grad_hidden_gates[2 * H_TILE + h_local] = grad_hidden_n;
                grad_input_gates_all[gate_offset + hid] = grad_input_r;
                grad_input_gates_all[gate_offset + HIDDEN_SIZE + hid] = grad_input_z;
                grad_input_gates_all[gate_offset + 2 * HIDDEN_SIZE + hid] = grad_input_n;
                grad_hidden_gates_all[gate_offset + hid] = grad_input_r;
            ...
            float recurrent_grad = 0.0f;
            float input_grad = 0.0f;
            const int own_row_begin = split_idx * H_TILE;
#pragma unroll
            for (int gate = 0; gate < 3; ++gate) {
                const float* weight_hh_gate = weight_hh + gate * HIDDEN_SIZE * HIDDEN_SIZE;
                const float* weight_ih_gate = weight_ih
                    + gate * HIDDEN_SIZE * (layer == 0 ? input_size : HIDDEN_SIZE);
                const float* sh_hidden_gate = sh_grad_hidden_gates + gate * H_TILE;
                const float* sh_input_gate = sh_grad_input_gates + gate * H_TILE;

                for (int row_local = group_lane; row_local < H_TILE; row_local += GROUP_SIZE) {
                    const int row = own_row_begin + row_local;
                    recurrent_grad += sh_hidden_gate[row_local]
                        * weight_hh_gate[row * HIDDEN_SIZE + hid];
                    if (layer != 0) {
                        input_grad += sh_input_gate[row_local]
                            * weight_ih_gate[row * HIDDEN_SIZE + hid];
                    } else if (hid < input_size) {
                        input_grad += sh_input_gate[row_local]
                            * weight_ih_gate[row * input_size + hid];
...
#pragma unroll
                for (int row_split = 0; row_split < SPLITS; ++row_split) {
                    if (row_split == split_idx) {
                        continue;
                    }
                    const int row_begin = row_split * H_TILE;
                    for (int row_local = group_lane; row_local < H_TILE; row_local += GROUP_SIZE) {
...
extern "C" __global__
void a100_gru_h256_backward_sequence_cooperative_split6_gate_cache_state_tiled_weight_shmem_split0_keep_unroll8_kernel(
    const float* __restrict__ grad_output,
    const float* __restrict__ gate_cache,
    const float* __restrict__ h0,
    const float* __restrict__ output,
    ...
    int seq_len)
{
    ...
    // split6_unroll8 只调整 unroll 因子，用于验证更多 ILP 是否抵消寄存器压力。
    float split0_partial_state = 0.0f;
...
extern "C" __global__
void a100_gru_h256_backward_sequence_cooperative_split8_gate_cache_state_tiled_weight_shmem_split0_keep_kernel(
    const float* __restrict__ grad_output,
    const float* __restrict__ gate_cache,
    const float* __restrict__ h0,
    const float* __restrict__ output,
    ...
    int seq_len)
{
    ...
    // split8 用更多 CTA 提高 batch=16 覆盖度，同时保持 split0 寄存器缓存自身 partial。
    float split0_partial_state = 0.0f;
