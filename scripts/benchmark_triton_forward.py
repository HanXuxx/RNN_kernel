#!/usr/bin/env python3
"""对 forward-only Triton GRU time-loop 原型做正确性和性能测试。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark Triton GRU forward prototype.")
...
def run_case(args: argparse.Namespace, hidden_size: int) -> dict[str, float | int | str]:
    ...
    with torch.no_grad():
        ...
        max_abs_diff = (torch_out - triton_out).abs().max().item()

        torch_ms = time_call(lambda: gru(x, h0), args.warmup_steps, args.timed_steps)
        triton_ms = time_call(
            lambda: triton_gru_forward_layer(
                x,
                h0[0],
                gru.weight_ih_l0,
                gru.weight_hh_l0,
    ...
    return {
        ...
        "torch_ms": torch_ms,
        "triton_ms": triton_ms,
        "speedup_vs_torch": torch_ms / triton_ms,
...
def main() -> None:
    ...
    for result in results:
        print(
            ...
            f"triton_ms={result['triton_ms']:.3f} "
