...
- `a100_gru_h256_backward_sequence_cooperative_split5_gate_cache_state_tiled_weight_shmem_split0_keep_kernel`
- `a100_gru_h256_backward_sequence_cooperative_split6_gate_cache_state_tiled_weight_shmem_split0_keep_kernel`
- `a100_gru_h256_backward_sequence_cooperative_split12_gate_cache_state_tiled_weight_shmem_split0_keep_kernel`
- `a100_gru_h256_backward_sequence_cooperative_split12_gate_cache_state_tiled_weight_shmem_split0_keep_unroll8_kernel`
...
- `a100_gru_forward_from_gates_cooperative_h256_htile4_compact_hoist_row4_prev_cache_shmem_gate_cache_kernel`
- `a100_gru_forward_from_gates_cooperative_h256_htile4_compact_hoist_row4_parallel_update_gate_cache_kernel`
- `a100_gru_forward_from_gates_cooperative_h256_htile4_compact_hoist_row4_ldg_shmem_gate_cache_kernel`
...
--implementation a100_gru_h256_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4
--implementation a100_gru_h256_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_prev_cache
--implementation a100_gru_h256_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_pack_prev
--implementation a100_gru_h256_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_parallel_update
...
--implementation a100_gru_h256_coop_split12_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_unroll8_htile4_compact_hoist_row4
...
34. `a100_gru_h256_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4`：
    把 backward 从 12 路 split 继续降到 6 路 split，每路处理 128 个 gate 项，并用约
    `131584` bytes dynamic shared memory 缓存 `128 x 256` recurrent weight tile。
    GPU3 seq8000 timed_steps=10 为 `93.408 ms/step`，forward `59.474 ms`，
    backward `33.347 ms`；Nsight Systems 中 backward 主 kernel 为 `28.531 ms`，
    grid 为 `96` blocks，dynamic shared memory 为 `131584` bytes，寄存器为
    `46/thread`。该实验说明在 A100 上用更多 shared memory 继续减少 partial 路数仍有效，
    是 hidden-prev pack 前的长序列最快训练路径。
    补充 split5 实验继续把 split 数降到 5，每路使用 ceil 后的 154 个 gate 项，并用约
    `158312` bytes dynamic shared memory 缓存 `154 x 256` recurrent weight tile；
    padding 行置零保证数学等价。GPU3 seq8000 timed_steps=3 为 `98.854 ms/step`，
    forward `60.165 ms`，backward `37.956 ms`，慢于 split6，说明继续逼近 shared
    memory 上限会让 tile 初始化、同步和调度成本超过减少一路 partial 的收益。
35. `a100_gru_h256_coop_split12_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4`：
    ...
    dot-product 并行度之间更平衡，是上一版最快训练路径。
36. `a100_gru_h256_coop_split12_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_unroll8_htile4_compact_hoist_row4`：
    在 split12 weight-shmem split0-keep 基础上把 shared-memory dot loop 和初始化 weight
    tile 的 loop 展开因子从 4 提到 8。GPU3 seq8000 timed_steps=3 为
    `94.249 ms/step`，timed_steps=10 复测为 `94.403 ms/step`，forward `59.449 ms`，
    backward `34.460 ms`。该分支没有稳定超过 split12 主线的 `94.286 ms/step`，说明
    当前 64 项 dot loop 的循环展开不是主要瓶颈。曾在该分支上短暂尝试用 `split_idx`
    简化 gate 索引，timed_steps=10 退化到 `98.093 ms/step`，已撤回。
37. `a100_gru_h256_coop_split24_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4`：
...
| A100 custom autograd，cooperative split6 persistent-state gate-cache tiled weight-shmem split0-keep htile4 compact hoist row4（GPU3 block=256） | 92.951 | 59.252 | 33.337 | 0.150 | 2.11 GB |
| A100 custom autograd，cooperative split6 persistent-state gate-cache tiled weight-shmem split0-keep htile4 compact hoist row4（GPU3 block=256 timed_steps=10） | 93.408 | 59.474 | 33.347 | 0.149 | 2.11 GB |
| A100 custom autograd，cooperative split6 persistent-state gate-cache tiled weight-shmem split0-keep htile4 compact hoist row4_ldg（GPU3 block=256） | 103.081 | 68.936 | 33.251 | 0.149 | 2.18 GB |
| A100 custom autograd，cooperative split6 persistent-state gate-cache tiled weight-shmem split0-keep htile4 compact hoist row4 parallel-update（GPU3 block=256） | 96.422 | 62.484 | 33.355 | 0.148 | 2.18 GB |
| A100 custom autograd，cooperative split6 persistent-state gate-cache tiled weight-shmem split0-keep htile4 compact hoist row4 prev-cache（GPU3 block=256） | 95.323 | 62.040 | 32.860 | 0.148 | 2.18 GB |
| A100 custom autograd，cooperative split6 persistent-state gate-cache tiled weight-shmem split0-keep htile4 compact hoist row4 pack-prev（GPU3 block=256 timed_steps=10） | 93.185 | 59.801 | 33.002 | 0.148 | 2.18 GB |
| A100 custom autograd，cooperative split5 persistent-state gate-cache tiled weight-shmem split0-keep htile4 compact hoist row4（GPU3 block=256） | 98.854 | 60.165 | 37.956 | 0.148 | 2.18 GB |
...
| A100 custom autograd，cooperative split12 persistent-state gate-cache tiled weight-shmem split0-keep unroll8 htile4 compact hoist row4（GPU3 block=256） | 94.249 | 59.354 | 34.454 | 0.150 | 2.11 GB |
| A100 custom autograd，cooperative split12 persistent-state gate-cache tiled weight-shmem split0-keep unroll8 htile4 compact hoist row4（GPU3 block=256 timed_steps=10） | 94.403 | 59.449 | 34.460 | 0.149 | 2.11 GB |
...
| htile4 compact hoist row4 shmem gate-cache tiled split6 weight-shmem split0-keep backward | 256 | 93.408 | 59.474 | 33.347 | 旧主线最快 |
| htile4 compact hoist row4 shmem gate-cache tiled split6 weight-shmem split0-keep backward + hidden-prev pack | 256 | 93.185 | 59.801 | 33.002 | 当前长序列最快 |
| htile4 compact hoist row4 shmem gate-cache tiled split12 weight-shmem split0-keep backward | 256 | 94.286 | 59.349 | 34.450 | 上一版最快 |
| htile4 compact hoist row4 shmem gate-cache tiled split12 weight-shmem split0-keep unroll8 backward | 256 | 94.403 | 59.449 | 34.460 | dot loop unroll8 未稳定超过主线 |
...
`htile4_compact_hoist_row4 block=256 + split6 weight-shmem split0-keep backward + hidden-prev pack`。split8
backward 说明过度减少 split 数会损失 recurrent dot-product 并行度，但 weight-shmem 后
split6 仍能启动，且用更高 shared memory 占用换来了更少 partial 路数；split24 说明继续
...
`34.450 ms`；split6 继续降到 `93.408 ms/step`，backward `33.347 ms`。hidden-prev
pack 只替换 backward 开始的 `torch.cat(...).transpose().contiguous()` 布局准备，
同场 t10 从 `93.644 ms/step` 降到 `93.185 ms/step`，backward 从 `33.340 ms`
降到 `33.002 ms`。Nsight
Systems 单 step 显示 split6 backward kernel 为 `28.531 ms`，dynamic shared memory 为
`131584` bytes，寄存器为 `46/thread`。own-shmem、unroll8 和 split24 均没有超过 split6
主线。
...
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_seq8000_bt256_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_seq8000_bt256_t10_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_seq8000_bt256_rerun_after_split5_ldg_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_seq8000_bt256_breakdown_after_pack_switch_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_seq8000_bt256_t10_breakdown_after_pack_switch_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_ldg_seq8000_bt256_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_ldg_seq8000_bt256_breakdown_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_prev_cache_seq8000_bt256_breakdown_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_pack_prev_seq8000_bt256_breakdown_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_pack_prev_seq8000_bt256_t10_breakdown_gpu3.csv`
- `results/a100_h256_train_custom_coop_split6_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_parallel_update_seq8000_bt256_breakdown_gpu3.csv`
- `results/a100_h256_train_custom_coop_split5_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_htile4_compact_hoist_row4_seq8000_bt256_breakdown_gpu3.csv`
...
- `results/a100_h256_train_custom_coop_split12_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_unroll8_htile4_compact_hoist_row4_seq8000_bt256_gpu3.csv`
- `results/a100_h256_train_custom_coop_split12_persistent_state_gate_cache_tiled_weight_shmem_split0_keep_unroll8_htile4_compact_hoist_row4_seq8000_bt256_t10_gpu3.csv`
...
34. split12 weight-shmem split0-keep 把每路 split 的 gate 项从 48 增到 64，
    ...
    `59.349 ms`，backward `34.450 ms`，是当时最快训练路径。
35. split12 weight-shmem split0-keep unroll8 把 64 项 shared-memory dot loop 的展开
    因子从 4 调到 8。GPU3 timed_steps=10 为 `94.403 ms/step`，backward
    `34.460 ms`，没有稳定超过 split12 主线；额外 gate 索引简化曾退化到
    `98.093 ms/step`，已撤回。
36. split24 weight-shmem split0-keep 把每路 split 的 gate 项降到 32。GPU3
    timed_steps=3 为 `105.399 ms/step`，backward `45.207 ms`，说明 split 数继续增加
    会放大 cooperative blocks 和 partial 规约成本，是负向边界实验。
37. row4 forward weight-shmem 把每个 forward CTA 的 `3 x 64 x 64` recurrent weight
...
38. row4 forward hidden-shmem 每步把 `hidden[k_begin:k_begin+64]` 放入 shared
...
39. qwarp forward 把 row 粒度从 half-warp 降到 quarter-warp，GPU1 timed_steps=3 为
    `191.674 ms/step`，forward `110.359 ms`。8-lane dot-product 丢掉 row4 的 hidden
    复用，且每个 lane 的 k 循环更长，是明确负向实验。
40. row3 forward 先算 3 行再算 1 行，目标是在 row4 hidden 复用和寄存器压力之间折中。
...
- `results/nsys/a100_h256_gate_cache_tiled_weight_shmem_split6_split0_keep_htile4_compact_hoist_row4_seq8000_bt256_range_2024.nsys-rep`
- `results/nsys/a100_h256_gate_cache_tiled_weight_shmem_split6_split0_keep_htile4_compact_hoist_row4_seq8000_bt256_range_2024.sqlite`
...
split6 split0-keep 的同类报告中，profile step 为 `92.816 ms`。seq8000 单 step 的
`cuda_gpu_kern_sum` 关键结果：

| kernel | total ms | time |
| --- | ---: | ---: |
| `a100_gru_forward_from_gates_cooperative_h256_htile4_compact_hoist_row4_shmem_gate_cache_kernel` | 58.004 | 62.5% |
| `a100_gru_h256_backward_sequence_cooperative_split6_gate_cache_state_tiled_weight_shmem_split0_keep_kernel` | 28.531 | 30.7% |

seq8000 的 split6 launch 形状：

| kernel | grid | block | regs/thread | dyn smem |
| --- | ---: | ---: | ---: | ---: |
| forward htile4 compact hoist row4 gate-cache | `256x1x1` | `256x1x1` | 74 | 768 |
| backward split6 gate-cache tiled weight-shmem split0-keep | `96x1x1` | `256x1x1` | 46 | 131584 |
...
`41.307 ms`，再由 split12 split0-keep 降到 `29.876 ms`，并由 split6 降到
`28.531 ms`。tiled backward 把动态 shared
...
dynamic shared memory 增到 `65792` 字节和寄存器到 `47/thread`。split6 继续把
cooperative grid 降到 `96` blocks，dynamic shared memory 增到 `131584` 字节，
但 backward kernel 仍继续下降到 `28.531 ms`。`grad_weight_hh`
...
34. **已完成 split6 weight-shmem split0-keep 正向实验**：把 backward split 数从
    12 继续降到 6，每路处理 128 个 gate 项，并使用约 `131584` bytes dynamic shared
    memory 缓存 recurrent weight tile。GPU3 seq8000 timed_steps=10 为
    `93.408 ms/step`，forward `59.474 ms`，backward `33.347 ms`；Nsight Systems
    backward kernel 为 `28.531 ms`，dyn smem 为 `131584` bytes，是 hidden-prev pack
    前的最快训练路径。
35. **已完成 split5 weight-shmem split0-keep 负向实验**：每路使用 154 个 gate 项和
    `158312` bytes dynamic shared memory，GPU3 seq8000 timed_steps=3 为
    `98.854 ms/step`，backward `37.956 ms`，说明继续降低 split 数会被过大的
    shared-memory tile 与 padding/调度成本抵消。
36. **已完成 split12 weight-shmem split0-keep 正向实验**：把 backward split 数从
    ...
    backward kernel 为 `29.876 ms`，dyn smem 为 `65792` bytes，是上一版最快训练路径。
37. **已完成 split12 unroll8 中性偏负实验**：把 split12 dot loop 展开因子从 4 提到
    8 后，GPU3 seq8000 timed_steps=10 为 `94.403 ms/step`，backward `34.460 ms`，
    没有稳定超过 split12 主线。额外 gate 索引简化曾退化到 `98.093 ms/step`，已撤回。
38. **已完成 split24 weight-shmem split0-keep 负向实验**：把 split 数增到 24 后，
    GPU3 seq8000 timed_steps=3 为 `105.399 ms/step`，backward `45.207 ms`，说明更小
    shared tile 没有抵消更多 cooperative blocks 和 partial 规约成本。
39. **已完成 row4 forward ldg 负向实验**：对只读的 input/weight/bias/h0 显式使用
    `__ldg`，并用 `__launch_bounds__(256, 3)` 把寄存器从原始 154/thread 压到
    67/thread 后，GPU3 seq8000 timed_steps=3 仍为 `103.081 ms/step`，forward
    `68.936 ms`；普通 row4 是 74/thread 和约 `59.5 ms` forward，说明 ldg/调度或
    spill 代价超过只读缓存收益。
40. **已完成 row4 forward prev-cache 负向实验**：forward 额外写出 time-major
    `hidden_prev_cache`，让 backward 跳过 `torch.cat(...).transpose().contiguous()`
    构造 `hidden_prev_steps`。GPU3 seq8000 timed_steps=3 为 `95.323 ms/step`，
    forward `62.040 ms`，backward `32.860 ms`；backward 比同窗口主线少约
    `0.49 ms`，但 forward 多约 `2.1 ms`，整体负向。kernel 寄存器为 `80/thread`，
    active blocks/SM 仍为 `3`。
41. **已完成 hidden-prev pack 正向实验**：保留旧 row4 forward，不在 forward
    额外写 cache；backward 开始用 `float4` CUDA kernel 直接把 `h0` 和
    `output[:, :-1]` 打包为 `[time, batch, hidden]`。GPU3 seq8000 同场
    timed_steps=3 从旧主线 `93.760 ms/step`、backward `33.348 ms` 降到
    `93.276 ms/step`、backward `32.999 ms`；timed_steps=10 从旧主线
    `93.644 ms/step`、backward `33.340 ms` 降到 `93.185 ms/step`、backward
    `33.002 ms`。该路径保留旧实现名，并新增 `..._pack_prev` 实现名用于 A/B 复测。
42. **已完成 row4 forward parallel-update 负向实验**：保持 row4 recurrent dot，
    但把 k0 partial 也写到 global，让 4 个 k CTA 分摊 gate/update/cache。
    GPU3 seq8000 timed_steps=3 为 `96.422 ms/step`，forward `62.484 ms`；
    kernel 寄存器为 `78/thread`，dynamic shared memory 为 `0`，active blocks/SM
    仍为 `3`。多写/多读 k0 partial 的成本超过分摊 pointwise/SFU 的收益。
43. **已完成 row4 forward weight-shmem 负向实验**：forward recurrent weight tile
    进 shared memory 后，seq8000 timed_steps=3 为 `111.147 ms/step`，forward
    `64.253 ms`，慢于同窗口 baseline。
44. **已完成 row4 forward hidden-shmem 负向实验**：每步 hidden k tile 进 shared
    memory 后，seq8000 timed_steps=3 为 `122.660 ms/step`，forward `75.952 ms`。
45. **已完成 qwarp forward 负向实验**：quarter-warp 后 seq8000 timed_steps=3 为
    `191.674 ms/step`，forward `110.359 ms`，说明不能牺牲 row4 的 hidden 复用。
46. **已完成 row3 forward 负向实验**：batch16 下 resident block 上限为 `216`，
    低于目标 `256`，说明 3 行同时计算仍超过寄存器/occupancy 边界。

下一步应优先研究：

1. 继续优化当前长序列最佳的 htile4 compact hoist row4 + split6 weight-shmem split0-keep backward
   ...
   缓存、ldg、prev-cache、parallel-update、qwarp 和 row3 都已验证为负向，hidden-prev
   pack 是当前唯一正向的布局准备优化。
2. 继续优化 weight-shmem backward：重点是 split6 的 shared-memory tile bank 行为、
   131.6KB dynamic shared memory 下的 resident block 上限、partial 规约和 grid sync 成本；
   split5 已证明 158.3KB tile 太重，不再优先继续降低 split 数。
3. 评估 forward/backward 联合 reserve-space 布局：当前 `gate_cache` 已换来速度，
   ...
   hidden-prev pack 已减少一次布局搬运，后续可以考虑更接近 cuDNN reserve-space 的布局，
   继续减少全局读写与转置。
4. 降低或压缩 gate-cache reserve-space：当前峰值显存约 `2.17 GB`，仍远低于 A100
   80GB 容量；可以继续用额外显存换速度，但要通过 Nsight 验证不是纯带宽放大。
5. 不再优先扫描 split 数：split24 太多，split12 是上一版甜点，split5 又太重，
   当前最优点落在 split6；后续只应在 split6 内部优化 shared-memory/partial 路径。
...
hoist row4 forward + split6 weight-shmem split0-keep backward，再加 hidden-prev pack，
把长序列 timed_steps=10 推到 `93.185 ms/step`。以同卡 cuDNN `247.279 ms/step` 为基准，
当前长序列最快路径约 `2.65x` 更快；相比同窗口 split12 split0-keep t10 对照
`94.286 ms/step` 约快 `1.2%`；相比同场旧 split6 主线 `93.644 ms/step`
约快 `0.5%`，backward 从 `33.340 ms` 降到 `33.002 ms`。split5、row4_ldg、
row4_prev_cache 和 row4_parallel_update 的最新结果说明，单纯继续增大 shared-memory
tile、显式走只读加载、在 forward 额外保存 hidden_prev，或把 pointwise 分散到更多 k CTA，
都不能突破当前瓶颈；但把 hidden_prev 布局准备改成 backward 端一次 vectorized pack
可以稳定拿回约 `0.3-0.5 ms`。瓶颈已经从 Python/autograd 逐步 launch 外层组织，
转向 row4 forward cooperative kernel、split6 weight-shmem backward 的 shared-memory
tile 细节、剩余同步、partial 规约读写和显式保存中间量带来的显存/带宽成本。
